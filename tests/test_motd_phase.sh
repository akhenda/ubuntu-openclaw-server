...
test_motd_phase_dry_run_writes_status_script() {
  ...
  assert_contains "$output_file" "[motd] configuring dynamic MOTD status script"
  assert_contains "$output_file" "[motd] [dry-run] would update /etc/update-motd.d/99-openclaw-status"
}
...
test_motd_phase_can_be_disabled() {
  ...
  assert_contains "$output_file" "MOTD_ENABLE=false; skipping MOTD setup"
  assert_not_contains "$output_file" "/etc/update-motd.d/99-openclaw-status"
}
