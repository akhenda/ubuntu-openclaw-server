...
test_ensure_hub_script_contains_routes_and_homepage_runtime() {
  ...
  assert_contains_text "$ensure_hub_script" '"networks": [edge_network_name]'
  assert_contains_text "$ensure_hub_script" '"/proc:/proc:ro"'
  assert_contains_text "$ensure_hub_script" 'traefik.http.routers.{kula_service_name}.rule=Host("{kula_host}")'
  assert_contains_text "$ensure_hub_script" 'traefik.http.services.{kula_service_name}.loadbalancer.server.port={kula_port}'
  ...
  assert_contains_text "$ensure_hub_script" 'PathPrefix("/api")'
  assert_contains_text "$ensure_hub_script" 'middlewares={mission_control_backend_service_name}-strip-api-prefix'
  assert_contains_text "$ensure_hub_script" 'stripprefix.prefixes=/api'
  ...
  assert_contains_text "$ensure_hub_script" '"NEXT_PUBLIC_API_URL": mission_control_frontend_api_url'
