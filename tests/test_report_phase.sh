...
APPS_ENABLE=false
REPORT_ENABLE=true
REPORT_SCRIPT=${edge_root}/bin/report.sh
REPORT_FAIL_ON_SEND=false
REPORT_OWNER_NAME=Hendaz
...
test_report_phase_dry_run_generates_helper() {
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  ...
  assert_contains "$output_file" "[report] configuring deployment report helper"
  assert_contains "$output_file" "[report] [dry-run] would update ${edge_root}/bin/report.sh"
  assert_contains "$output_file" "REPORT_TARGET is not set"
  assert_contains "$output_file" "[report] report helper setup complete"
}

test_report_phase_can_be_disabled() {
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  trap '[[ -n "${tmp_dir:-}" ]] && rm -rf "${tmp_dir}"' RETURN
  ...
  assert_contains "$output_file" "REPORT_ENABLE=false; skipping report helper setup"
  assert_not_contains "$output_file" "report helper setup complete"
}

main() {
  test_report_phase_dry_run_generates_helper
  test_report_phase_can_be_disabled
  echo "PASS: test_report_phase.sh"
}
