...
test_socket_proxy_phase_enabled_message() {
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  ...
  assert_contains "$output_file" "[socket-proxy] socket proxy is enabled; edge phase will deploy docker-socket-proxy"
}

test_socket_proxy_can_be_disabled() {
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  trap '[[ -n "${tmp_dir:-}" ]] && rm -rf "${tmp_dir}"' RETURN
  ...
  assert_contains "$output_file" "SOCKET_PROXY_ENABLE=false; skipping docker socket proxy phase"
}

main() {
  test_socket_proxy_phase_enabled_message
  test_socket_proxy_can_be_disabled
  echo "PASS: test_socket_proxy_phase.sh"
}
