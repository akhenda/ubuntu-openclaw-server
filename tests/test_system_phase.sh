...
test_system_phase_dry_run_renders_baseline_operations() {
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  ...
  assert_contains "$output_file" "[system] applying hostname/timezone/unattended/fail2ban baseline"
  assert_contains_one_of "$output_file" "timedatectl set-timezone Africa/Nairobi" "/usr/share/zoneinfo/Africa/Nairobi"
  assert_contains_one_of "$output_file" "hostnamectl set-hostname node.akhenda.net" "hostname node.akhenda.net"
  assert_contains "$output_file" "[system] [dry-run] would update /etc/hostname"
  assert_contains "$output_file" "[system] [dry-run] would update /etc/hosts"
  assert_contains "$output_file" "[system] [dry-run] would update /etc/apt/apt.conf.d/50unattended-upgrades"
  assert_contains "$output_file" "[system] [dry-run] would update /etc/fail2ban/jail.d/openclaw.local"
}

test_system_phase_requires_host_fqdn() {
  local tmp_dir
  tmp_dir="$(mktemp -d)"
  trap '[[ -n "${tmp_dir:-}" ]] && rm -rf "${tmp_dir}"' RETURN
  ...
  if [[ $rc -eq 0 ]]; then
    echo "Assertion failed: expected missing HOST_FQDN config validation error" >&2
    cat "$output_file" >&2
    exit 1
  fi

  assert_contains "$output_file" "Missing required variable: HOST_FQDN"
}

main() {
  test_system_phase_dry_run_renders_baseline_operations
  test_system_phase_requires_host_fqdn
  echo "PASS: test_system_phase.sh"
}
